...
import com.util.HtmlButton;
import com.util.IConnectionConfiguration;
import com.util.IConnectionConfiguration.DefaultConnectionConfiguration;
import com.util.NewLookAndFeel;
import com.util.NewTabbedPanel;
...
public class ApiDemo implements IConnectionHandler
{
    ...
    private class ConnectionPanel extends JPanel
    {
        ...
        private final JTextField m_connectOptionsTF = new JTextField(m_connectionConfiguration
                                                                             .getDefaultConnectOptions(), 30);
        ...
        private final JLabel m_defaultPortNumberLabel = new JLabel(
                ...
                "Simulated Trading ports for new installations of " + "version 954.1 or newer: " +
...
// probably should not send F..A position updates to listeners, at least not to API; also probably not send FX
// positions; or maybe we should support that?; filter out models or include it
...
// When API requests SSF contracts, TWS sends both dividend protected and non-dividend protected contracts. They are
// indistinguishable except for having different conids.
...
// When an order is submitted with the "End time" field, it seems to be ignored; it is not submitted but also no
// error is returned to API.
// Most error messages from TWS contain the class name where the error occurred which gets garbled to gibberish
// during obfuscation; the class names should be removed from the error message
// If you exercise option from API after 4:30, TWS pops up a message; TWS should never popup a message due to an API
// request
...
// When a child order is transmitted with a different contract than the parent but no hedge params it sort of works
// but not really, e.g. contract does not display at TWS, but order is working
// Switch between frozen and real-time quotes is broken; e.g.: request frozen quotes, then realtime, then request
// option market data; you don't get bid/ask; request frozen, then an option; you don't get anything
...
// reqContractDetails needs primary exchange, currently only takes currency which is wrong; all requests taking
// Contract should be updated
// reqMktDepth and reqContractDetails does not take primary exchange but it needs to; ideally we could also pass
// underConid in request
...
//TWS sends error "Snapshot exceeds 100/sec" even when it doesn't; maybe try flush? or maybe send 100 then pause 1
// second? this will take forever; i think the limit needs to be increased
...
//Submitting an initial stop price for trailing stop orders is supported only for trailing stop-limit orders; should
// be supported for plain trailing stop orders as well
