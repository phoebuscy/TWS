...
import com.util.NewTabbedPanel.INewTab;
import com.util.TCombo;
import com.util.UpperField;
import com.util.VerticalPanel;
import com.util.VerticalPanel.HorzPanel;


public class ComboPanel extends JPanel implements INewTab {
	private final OrdersPanel.OrdersModel m_ordersModel = new OrdersPanel.OrdersModel() {
	...
	protected void onDoubleClick(int row) {
		if (row != -1) {
			OrdersPanel.OrderRow order = m_ordersModel.get(row);
	...
	static class EfpPanel extends JPanel {
		...
		static class EfpModel extends AbstractTableModel {
			...
			class EfpRow extends TopModel.TopRow implements IEfpHandler {
